...
class LP_Appraisal(models.Model):
    ...
    def _compute_total_salary(self):
        for rec in self:
            rec.lp_total_salary = rec.lp_salary + rec.lp_salary_raise    
...
class LP_Hremployee(models.Model):
    _inherit = 'hr.employee'
    _description = 'Archive employee'

    def write(self, vals):
        if  vals.get('active') == False:
            appraisales = self.env['hr.appraisal'].sudo().search(
                [('employee_id', 'in', self.ids)])
            appraisales.write({
                'active': False
            })

        return super(LP_Hremployee, self).write(vals)
